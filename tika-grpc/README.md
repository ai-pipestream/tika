...
This is server-global. It raises what *every* RPC will accept, including the
management calls that only ever carry small JSON. A unary message is held in
full in memory before the handler runs, so the value also bounds the heap one
...
- **`org.apache.tika.grpc.v2.TikaV2` (experimental)** — typed parse replies that
  ...
  Includes a TIKA-4795 **ParseBytes PoC** (parse-only bytes in, same `Document`
  out). Implementation details of ParseBytes are intentionally open for the
  StormCrawler / crawler discussion; this branch demonstrates the additive v2
  shape.
